...
<p class = "descrip"><i>Must enter correct old common name!</i></p>

<form class = 'formstyle' action='' method = 'POST'>
Old common name: <input type='text' name='old'><br>
New common name: <input type='text' name='new'><br>
...
<?php
    ...
    {
        $db2 = new MyDB();
        $check = "SELECT COUNT(*) FROM FLOWERS WHERE COMNAME LIKE '$old'";
        $run = $db2->query($check);
    ...
    {
        $db = new MyDB();
        $sql = "UPDATE FLOWERS SET COMNAME = '$new' WHERE COMNAME LIKE '$old'";
        $ret = $db->exec($sql);
        $db->close();
        echo "<p class = 'styled' > The flower's common name has been updated! </p>";
    }
    ?>
